...
function QuestionsInner() {
  const { state, addQuestion } = useMasterOS();
  ...
  const questions = useMemo(
    () => state.questions.filter((item) => !filterSkill || item.skillId === filterSkill),
    [state.questions, filterSkill],
  );
    ...
    <div className="mos-page">
      <div className="mos-top">
        <div>
          ...
          <h1>Question Bank</h1>
          <p>Add once, then drop into lessons, homework, quizzes, warm-ups, and exit tickets.</p>
        </div>
      ...
      <div className="mos-actions" style={{ marginBottom: 16 }}>
        <select className="mos-ghost" value={filterSkill} onChange={(e) => setFilterSkill(e.target.value)}>
          <option value="">All skills</option>
          {state.skills.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
        </select>
      </div>
      ...
      <div className="mos-list-page">
        {questions.map((question) => {
          ...
          return (
            <article key={question.id} className="mos-entity">
