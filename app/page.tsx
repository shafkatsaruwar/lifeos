      ...
      <main>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={view} className={`page ${view === "Life" || view === "School" || view === "Work" || view === "Study Abroad" ? "os-page" : ""}`} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: .18 }}>
            ...
            {view === "Work" && <WorkDashboard tasks={tasks} projects={projectItems.filter(p => p.scope === "work")} work={workHub} onComplete={complete} onOpenTask={setTaskPageId} onOpenProject={(name) => { setSpacesFilter("Projects"); setSelectedClassId(null); setSelectedProjectName(name); setView("Spaces"); }} onNewTask={() => setComposer("task")} onNewProject={() => { setCreationScope("work"); setComposer("project"); }} onOpenCollection={(key: WorkHubKey) => setHubCollection({ scope: "work", key })} onOpenNow={() => go("Now")} />}
