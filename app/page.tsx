...
import {
  ...
  Ban, Pencil, Trash2, Bell, Database, Download, Palette, Shield, SlidersHorizontal, Coffee,
} from "lucide-react";
...
const initialTasks: Task[] = [
  { id: 1, title: "Finalize onboarding flow", project: "Synapse", color: "#635bff", due: "Today", priority: "High", focusMinutes: 45, energy: "High" },
  { id: 2, title: "Edit Brooklyn portrait set", project: "Photography", color: "#e48b6b", due: "Today", priority: "Medium", focusMinutes: 60, energy: "Medium" },
  { id: 3, title: "Review personal statement draft", project: "Master's Applications", color: "#47a47b", due: "Today", priority: "High", focusMinutes: 30, energy: "High" },
  { id: 4, title: "Update catering menu pricing", project: "Mom's Catering", color: "#d99b38", due: "Tomorrow", priority: "Medium", focusMinutes: 25, energy: "Medium" },
  { id: 5, title: "Prepare portfolio case study", project: "Career", color: "#4e8bd7", due: "Jul 12", priority: "Low", focusMinutes: 45, energy: "Low" },
];
...
export default function LifeOS() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedSettings) setSettingsState({ ...initialSettings, ...JSON.parse(savedSettings) });
    } catch {
      window.localStorage.removeItem(SETTINGS_STORAGE_KEY);
      window.localStorage.removeItem(DARK_STORAGE_KEY);
    } finally {
  ...
  const addProject = (name: string, kind: ProjectKind = "finishable") => {
    setProjectItems(items => [...items, { name, desc: kind === "maintenance" ? "An ongoing system to keep healthy." : "A focused project with a finish line.", progress: kind === "maintenance" ? 100 : 0, color: "#625af6", icon: FolderKanban, iconName: "FolderKanban", tasks: 0, kind }]);
    setComposer(null);
  ...
  const updateTask = (id: number, updates: Pick<Task, "title" | "focusMinutes" | "energy" | "project" | "color">) => {
  ...
  return (
    <div className="app-shell">
      ...
      <AnimatePresence>
        ...
        {calendarComposer && <CalendarEventModal key="calendar-event-modal" close={() => setCalendarComposer(false)} add={addCalendarEvent} />}
        ...
        {actionProjectName && projectItems.find(project => project.name === actionProjectName) && <ProjectActionsModal key={`project-actions-${actionProjectName}`} project={projectItems.find(project => project.name === actionProjectName)!} close={() => setActionProjectName(null)} setKind={(kind) => updateProjectKind(actionProjectName, kind)} />}
        {selectedProjectName && projectItems.find(project => project.name === selectedProjectName) && <ProjectDetailModal key={`project-detail-${selectedProjectName}`} project={projectItems.find(project => project.name === selectedProjectName)!} tasks={tasks} close={() => setSelectedProjectName(null)} linkTask={linkTaskToProject} />}
  ...
  return <>
    ...
    <div className="dashboard-grid">
      <section className="card priorities">
        ...
        <div className="task-list">{tasks.length ? tasks.slice(0, 3).map((task, i) => <div className={`task-row ${task.done ? "done" : ""} ${task.canceled ? "canceled" : ""}`} key={task.id}><button className="check" disabled={task.canceled} onClick={() => onComplete(task.id)}>{task.done ? <Check size={14} /> : task.canceled ? <X size={13} /> : <span>{i + 1}</span>}</button><button className="task-open" disabled={task.canceled} onClick={() => onFocus(task.id)}><strong>{task.title}</strong><p><i style={{ background: task.color }} />{task.project} <span>·</span> {task.canceled ? "Canceled" : task.due} <span>·</span> {task.focusMinutes} min <span>·</span> {task.energy} energy</p></button><button className="quick-focus" disabled={task.canceled} aria-label={`Focus on ${task.title}`} title="Focus on this task" onClick={() => onFocus(task.id)}><Focus size={15} /></button><button className="more" aria-label={`Actions for ${task.title}`} onClick={() => onTaskMenu(task.id)}><MoreHorizontal size={17} /></button></div>) : <div className="priority-empty"><strong>No priorities right now.</strong><p>Clean slate. Add one when your brain hands you the next thing.</p></div>}</div>
        <button className="add-row" onClick={onNewTask}><Plus size={15} /> Add priority</button>
      </section>
      <section className="focus-card">
        <div className="focus-glow" />
        <div className="card-head"><div><span className="section-icon dark-icon"><Focus size={14} /></span><h2>Next focus</h2></div><span className="ready"><i /> Ready</span></div>
        <div className="focus-content"><p>{nextTask?.project ?? "Nothing queued"}</p><h3>{nextTask?.title ?? "Add a priority to start focusing"}</h3><div className="focus-meta"><span><Clock3 size={14} /> {nextTask?.focusMinutes ?? 45} min</span><span><Flame size={14} /> {nextTask?.energy ?? "Medium"} energy</span></div></div>
        <button className="start-focus" onClick={() => nextTask ? onFocus(nextTask.id) : onNewTask()}><Focus size={16} /> {nextTask ? "Start focus session" : "Add first priority"} <span>{nextTask ? "F" : "N"}</span></button>
      </section>
...
function SettingsView({ dark, setDark, settings, update, tasks, projects, events, brainItems, flash }: { dark: boolean; setDark: (next: boolean | ((value: boolean) => boolean)) => void; settings: SettingsState; update: (updates: Partial<SettingsState>) => void; tasks: Task[]; projects: Project[]; events: CalendarEvent[]; brainItems: string[]; flash: (message: string) => void }) {
  ...
  const exportData = () => {
    const payload = { exportedAt: new Date().toISOString(), tasks, projects: projects.map(({ icon, ...project }) => project), events, brainItems, settings, dark };
...
function FocusMode({ task, tasks, onSwitch, onUpdateChecklist, onUpdateChecklistProgress, onComplete, onExit }: { task: Task; tasks: Task[]; onSwitch: (id: number) => void; onUpdateChecklist: (id: number, checklist: string[]) => void; onUpdateChecklistProgress: (id: number, progress: boolean[]) => void; onComplete: (id: number) => void; onExit: () => void }) {
  ...
  const addChecklistItem = () => saveChecklist([...checklist, "New focus step"]);
  ...
  return <motion.div className="focus-mode" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
    ...
    <main>
      ...
      <aside className="timer-panel"><span>FOCUS TIMER</span><div className="timer-ring" style={{ "--progress": `${(seconds / totalSeconds) * 360}deg` } as React.CSSProperties}><div><strong>{time}</strong><span>{task.focusMinutes} minute session</span></div></div><button className="timer-button" onClick={() => setRunning(v => !v)}>{running ? <><Circle size={15} fill="currentColor" /> Pause</> : <><Focus size={16} /> Begin session</>}</button><button className="reset" onClick={() => { setSeconds(totalSeconds); setRunning(false); }}><TimerReset size={15} /> Reset timer</button>
        ...
        <button className="mark-done-button" onClick={() => onComplete(task.id)}><CheckCircle2 size={16} /> Mark as done</button>
      </aside>
    </main>
...
function CalendarEventModal({ close, add, event, remove }: { close: () => void; add: (event: CalendarEvent) => void; event?: CalendarEvent; remove?: (id: string) => void }) {
  const now = new Date();
  const eventDay = event?.start.slice(0, 10) ?? toDateKey(now);
...
function EditTaskModal({ task, projects, close, save }: { task: Task; projects: Project[]; close: () => void; save: (updates: Pick<Task, "title" | "focusMinutes" | "energy" | "project" | "color">) => void }) {
  ...
  return <motion.div className="modal-layer" onMouseDown={close} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.form className="create-modal" onMouseDown={event => event.stopPropagation()} onSubmit={event => { event.preventDefault(); if (title.trim()) save({ title: title.trim(), project: linkedProject?.name ?? "Inbox", color: linkedProject?.color ?? "#625af6", focusMinutes: parsedMinutes, energy }); }} initial={{ scale: .98, y: 8 }} animate={{ scale: 1, y: 0 }}><div className="capture-head"><div className="brain-dot"><Pencil size={16} /></div><div><strong>Edit priority</strong><span>Update the project link, focus length, and energy cost.</span></div><button type="button" onClick={close} aria-label="Close"><X size={18} /></button></div><label htmlFor="edit-task-name">Priority name</label><input id="edit-task-name" autoFocus value={title} onChange={event => setTitle(event.target.value)} /><label htmlFor="edit-task-project">Linked project</label><select id="edit-task-project" value={projectName} onChange={event => setProjectName(event.target.value)}><option value="Inbox">Inbox / no project</option>{projects.map(project => <option key={project.name} value={project.name}>{project.name}</option>)}</select><div className="focus-edit-grid"><label htmlFor="edit-focus-minutes">Focus time<input id="edit-focus-minutes" type="number" min={5} max={240} step={5} value={focusMinutes} onChange={event => setFocusMinutes(event.target.value)} /></label><label>Energy needed<div className="energy-picker">{(["Low", "Medium", "High"] as EnergyLevel[]).map(level => <button key={level} type="button" className={energy === level ? "selected" : ""} onClick={() => setEnergy(level)}>{level}</button>)}</div></label></div><div className="create-actions"><button type="button" onClick={close}>Cancel</button><button className="primary" disabled={!title.trim()} type="submit">Save changes</button></div></motion.form></motion.div>;
}

function CreateModal({ kind, projects, close, submit }: { kind: "task" | "project"; projects: Project[]; close: () => void; submit: (value: string, projectKind?: ProjectKind, taskProject?: string) => void }) {
