...
import {
  WorkModeAdapter, SchoolModeAdapter, LifeModeAdapter, StudyAbroadModeAdapter,
} from "@/app/components/OperatingModes";
import type {
...
const routedViews: Record<string, View> = {
  life: "Life", school: "School", "study-abroad": "Study Abroad",
...
export default function LifeOS() {
  ...
  const [projectComposerName, setProjectComposerName] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    setStudyAbroadHydrated(false);
    (async () => {
      try {
        const data = await loadDataFromFirebase('study-abroad');
        if (data && typeof data === "object") {
          setStudyAbroadHub({ ...emptyStudyAbroadHub, ...data });
        } else {
          // First-time user: load seed data
          setStudyAbroadHub(createStudyAbroadSeedData());
        }
      } catch (error) {
  ...
  }, [cloudUserId, settingsHydrated, settingsState.enableStudyAbroad]);

  useEffect(() => {
    if (!cloudUserId || !studyAbroadHydrated || !settingsState.enableStudyAbroad) return;
    syncDataToFirebase('study-abroad', studyAbroadHub);
  }, [cloudUserId, studyAbroadHub, studyAbroadHydrated, settingsState.enableStudyAbroad]);
  ...
  const exportData = () => {
    const payload = { exportedAt: new Date().toISOString(), tasks, projects: projectItems.map(({ icon: _, ...project }) => project), events: calendarEvents, brainItems, classes, notes, resources, life: lifeHub, school: schoolHub, studyAbroad: studyAbroadHub, settings: settingsState, dark };
  ...
  if (fullscreenProject && projectItems.find(p => p.name === fullscreenProject)) {
    return <FullscreenProject project={projectItems.find(p => p.name === fullscreenProject)!} tasks={tasks} onExit={() => setFullscreenProject(null)} linkTask={linkTaskToProject} onOpenTask={setTaskPageId} />;
  }
  if (focus) {
    if (!activeTask) return <EmptyFocus onExit={() => { setFocus(false); setView("Now"); }} onNewTask={() => { setFocus(false); setComposer("task"); }} />;
    return <FocusMode key={activeTask.id} task={activeTask} tasks={activeTasks} onSwitch={(id) => { setActiveTaskId(id); chooseNowTask(id); const task = tasks.find(item => item.id === id); if (task) rememberSpaceTask(task); }} onUpdateChecklist={updateTaskChecklist} onUpdateChecklistProgress={updateTaskChecklistProgress} onUpdateFocusSession={updateFocusSession} onComplete={complete} onExit={(destination) => { setFocus(false); setView(destination ?? "Now"); }} />;
  }
    ...
    <div className="app-shell">
      ...
      <aside className={`sidebar ${sidebar ? "sidebar-open" : ""}`}>
        ...
        <nav>
          ...
          {nav.map(({ name, icon: Icon }) => <button key={name} className={`nav-item ${view === name ? "active" : ""}`} onClick={() => go(name)}><Icon size={17} strokeWidth={1.8} /><span>{name}</span>{name === "Brain" && <em>{brainItems.length}</em>}{name === "Spaces" && projectItems.length + classes.length > 0 && <em>{projectItems.length + classes.length}</em>}</button>)}
        </nav>
      ...
      <main>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={view} className={`page ${view === "Life" || view === "School" || view === "Work" || view === "Study Abroad" ? "os-page" : ""}`} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: .18 }}>
            {view === "Life" && <LifeModeAdapter tasks={tasks} projects={projectItems.filter(p => p.scope === "life")} notes={notes} life={lifeHub} onComplete={complete} onOpenTask={setTaskPageId} onOpenProject={(name) => { setSpacesFilter("Projects"); setSelectedClassId(null); setSelectedProjectName(name); setView("Spaces"); }} onNewTask={() => setComposer("task")} onNewProject={() => { setCreationScope("life"); setComposer("project"); }} onNewNote={() => createNote()} onOpenCollection={(key: LifeHubKey, startAdd) => setHubCollection({ scope: "life", key, startAdd })} />}
            {view === "School" && <SchoolModeAdapter tasks={tasks} classes={classes} school={schoolHub} onComplete={complete} onOpenTask={setTaskPageId} onOpenClass={(id) => { setSelectedProjectName(null); setSelectedClassId(id); setView("Spaces"); }} onNewCourse={() => setSpaceComposer("class")} onNewAcademic={() => classes.some(item => !isClassArchived(item)) ? setSchoolClassAction("coursework") : setSpaceComposer("class")} onNewLecture={() => classes.some(item => !isClassArchived(item)) ? setSchoolClassAction("lecture") : setSpaceComposer("class")} onOpenCollection={(key: SchoolHubKey, startAdd) => setHubCollection({ scope: "school", key, startAdd })} onOpenProfile={() => setSchoolProfileOpen(true)} onFocus={openFocus} enableStudyAbroad={settingsState.enableStudyAbroad} onToggleStudyAbroad={(enabled) => updateSettings({ enableStudyAbroad: enabled })} />}
            {view === "Work" && <WorkModeAdapter tasks={tasks} projects={projectItems.filter(p => p.scope === "work")} work={workHub} onComplete={complete} onOpenTask={setTaskPageId} onOpenProject={(name) => { setSpacesFilter("Projects"); setSelectedClassId(null); setSelectedProjectName(name); setView("Spaces"); }} onNewTask={() => setComposer("task")} onNewProject={() => { setCreationScope("work"); setComposer("project"); }} onOpenCollection={(key: WorkHubKey) => setHubCollection({ scope: "work", key })} onOpenNow={() => go("Now")} />}
            {view === "Study Abroad" && settingsState.enableStudyAbroad && <StudyAbroadModeAdapter hub={studyAbroadHub} onOpenUniversities={() => setStudyAbroadCollection("universities")} onOpenPrograms={() => setStudyAbroadCollection("programs")} onOpenApplications={() => setStudyAbroadCollection("applications")} onOpenScholarships={() => setStudyAbroadCollection("scholarships")} onOpenDocuments={() => setStudyAbroadCollection("documents")} />}
      ...
      <AnimatePresence>
        ...
        {palette && <CommandPalette key="command-palette" query={query} setQuery={setQuery} results={filtered} close={() => { setPalette(false); setQuery(""); }} go={go} onFocus={() => { setPalette(false); openFocus(); }} onCapture={() => { setPalette(false); setCapture(true); }} onNewTask={() => { setPalette(false); setComposer("task"); }} onResult={(result) => { const task = activeTasks.find(item => item.title === result); const project = projectItems.find(item => item.name === result); const classRecord = classes.find(item => item.code === result); const note = notes.find(item => item.title === result); setPalette(false); setQuery(""); if (task) setTaskPageId(task.id); else if (project) { setSelectedClassId(null); setSelectedProjectName(project.name); setView("Spaces"); } else if (classRecord) { setSelectedProjectName(null); setSelectedClassId(classRecord.id); setView("Spaces"); } else if (note) { setSelectedNoteId(note.id); setView("Notes"); } }} nav={nav} />}
        ...
        {weeklyReviewOpen && <WeeklyReviewModal key="weekly-review" tasks={tasks} projects={projectItems} classes={classes} close={() => { setWeeklyReviewOpen(false); setSettingsState(current => ({ ...current, weeklyReviewDate: toDateKey(new Date()) })); }} onOpenTask={setTaskPageId} onGoSpaces={() => { setWeeklyReviewOpen(false); go("Spaces"); }} />}
        ...
        {hubCollection?.scope === "work" && <HubCollectionModal key={`work-${hubCollection.key}`} target={hubCollection} records={workHub[hubCollection.key]} close={() => setHubCollection(null)} change={(records) => setWorkHub(current => ({ ...current, [hubCollection.key]: records }))} />}
        ...
        {composer && <CreateModal key={`create-${composer}`} kind={composer} projects={projectItems} classes={classes} close={() => { setComposer(null); setParentTaskIdForCreation(null); setProjectComposerName(null); }} submit={(value, projectKind, taskProject) => composer === "task" ? addTask(value, projectKind, taskProject || projectComposerName || undefined, parentTaskIdForCreation ?? undefined) : addProject(value, projectKind)} />}
        ...
        {actionTaskId !== null && tasks.find(task => task.id === actionTaskId) && <TaskActionsModal key={`task-actions-${actionTaskId}`} task={tasks.find(task => task.id === actionTaskId)!} close={() => setActionTaskId(null)} open={() => { setTaskPageId(actionTaskId); setActionTaskId(null); }} edit={() => { setEditingTaskId(actionTaskId); setActionTaskId(null); }} toggleCanceled={() => toggleCanceled(actionTaskId)} remove={() => deleteTask(actionTaskId)} />}
        ...
        {studyAbroadCollection && <StudyAbroadCollectionView key={`study-collection-${studyAbroadCollection}`} type={studyAbroadCollection} items={studyAbroadCollection === "universities" ? studyAbroadHub.universities : studyAbroadCollection === "programs" ? studyAbroadHub.programs : studyAbroadCollection === "applications" ? studyAbroadHub.applications : studyAbroadCollection === "scholarships" ? studyAbroadHub.scholarships : studyAbroadHub.documents} close={() => setStudyAbroadCollection(null)} onAdd={() => { setStudyAbroadCollection(null); if (studyAbroadCollection === "universities") setStudyAbroadModal("university"); else if (studyAbroadCollection === "programs") setStudyAbroadModal("program"); else if (studyAbroadCollection === "applications") setStudyAbroadModal("application"); else if (studyAbroadCollection === "scholarships") setStudyAbroadModal("scholarship"); else if (studyAbroadCollection === "documents") setStudyAbroadModal("document"); }} onEdit={(item) => { if (studyAbroadCollection === "universities") { setSelectedUniversity(item.id); setStudyAbroadModal("university"); } else if (studyAbroadCollection === "programs") setStudyAbroadModal("program"); else if (studyAbroadCollection === "documents") setStudyAbroadModal("document"); }} onDelete={(id) => { if (studyAbroadCollection === "universities") setStudyAbroadHub(current => ({ ...current, universities: current.universities.filter(u => u.id !== id) })); else if (studyAbroadCollection === "programs") setStudyAbroadHub(current => ({ ...current, programs: current.programs.filter(p => p.id !== id) })); else if (studyAbroadCollection === "applications") setStudyAbroadHub(current => ({ ...current, applications: current.applications.filter(a => a.id !== id) })); else if (studyAbroadCollection === "scholarships") setStudyAbroadHub(current => ({ ...current, scholarships: current.scholarships.filter(s => s.id !== id) })); else if (studyAbroadCollection === "documents") setStudyAbroadHub(current => ({ ...current, documents: current.documents.filter(d => d.id !== id) })); }} hub={studyAbroadHub} />}
        {studyAbroadModal === "university" && <UniversityModal university={selectedUniversity ? studyAbroadHub.universities.find(u => u.id === selectedUniversity) : undefined} close={() => { setStudyAbroadModal(null); setSelectedUniversity(null); }} save={(university) => { setStudyAbroadHub(current => ({ ...current, universities: selectedUniversity ? current.universities.map(u => u.id === university.id ? university : u) : [...current.universities, university] })); setStudyAbroadModal(null); setSelectedUniversity(null); }} />}
        {studyAbroadModal === "program" && <ProgramModal close={() => setStudyAbroadModal(null)} save={(program) => { setStudyAbroadHub(current => ({ ...current, programs: [...current.programs, program] })); setStudyAbroadModal(null); }} />}
...
function SpaceModal({ initialKind, close, addProject, addClass }: { initialKind: SpaceKind; close: () => void; addProject: (name: string, kind: ProjectKind, description: string, color: string, icon: ProjectIcon) => void; addClass: (value: Omit<ClassRecord, "id">) => void }) {
  ...
  return <motion.div className="modal-layer" onMouseDown={close} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.form className="create-modal space-modal" onMouseDown={event => event.stopPropagation()} onSubmit={submit} initial={{ scale: .98, y: 8 }} animate={{ scale: 1, y: 0 }}>
...
function ProjectDetailView({ project, tasks, onBack, onUpdateTask, onNewTask }: { project: Project; tasks: Task[]; onBack: () => void; onUpdateTask: (taskId: number, updates: Partial<Task>) => void; onNewTask: (projectName: string) => void }) {
  ...
  return <section className="class-detail-view">
    ...
    <div className="class-meta-strip">
      ...
      <div><span>Highest priority</span><strong>{topPriority}</strong></div>
...
function FocusMode({ task, tasks, onSwitch, onUpdateChecklist, onUpdateChecklistProgress, onUpdateFocusSession, onComplete, onExit }: { task: Task; tasks: Task[]; onSwitch: (id: number) => void; onUpdateChecklist: (id: number, checklist: string[]) => void; onUpdateChecklistProgress: (id: number, progress: boolean[]) => void; onUpdateFocusSession: (id: number, session: FocusSessionUpdate) => void; onComplete: (id: number) => void; onExit: (destination?: View) => void }) {
...
function CommandPalette({ query, setQuery, results, close, go, onFocus, onCapture, onNewTask, onResult, nav }: { query: string; setQuery: (s: string) => void; results: string[]; close: () => void; go: (v: View) => void; onFocus: () => void; onCapture: () => void; onNewTask: () => void; onResult: (result: string) => void; nav: { name: View; icon: any }[] }) {
