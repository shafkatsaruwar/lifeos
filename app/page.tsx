...
export default function LifeOS() {
  ...
  const [workOSModalOpen, setWorkOSModalOpen] = useState<"applications" | "skills" | "goals" | null>(null);
  ...
  return (
    <div className="app-shell">
      ...
      <AnimatePresence>
        ...
        {workOSModalOpen === "applications" && <WorkOSApplicationsModal items={workHub.applications} close={() => setWorkOSModalOpen(null)} add={(app) => setWorkHub(current => ({ ...current, applications: [app, ...current.applications] }))} update={() => {}} remove={(id) => setWorkHub(current => ({ ...current, applications: current.applications.filter(a => a.id !== id) }))} />}
        {workOSModalOpen === "skills" && <WorkOSSkillsModal items={workHub.skills} close={() => setWorkOSModalOpen(null)} add={(skill) => setWorkHub(current => ({ ...current, skills: [skill, ...current.skills] }))} remove={(id) => setWorkHub(current => ({ ...current, skills: current.skills.filter(s => s.id !== id) }))} />}
        {workOSModalOpen === "goals" && <WorkOSGoalsModal items={workHub.goals} close={() => setWorkOSModalOpen(null)} add={(goal) => setWorkHub(current => ({ ...current, goals: [goal, ...current.goals] }))} remove={(id) => setWorkHub(current => ({ ...current, goals: current.goals.filter(g => g.id !== id) }))} />}
