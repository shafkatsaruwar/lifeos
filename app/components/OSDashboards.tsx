...
export function HubCollectionModal({ target, records, close, change }: { target: HubCollectionTarget; records: HubRecord[]; close: () => void; change: (records: HubRecord[]) => void }) {
  ...
  const add = (event: React.FormEvent) => { event.preventDefault(); if (!title.trim()) return; change([{ id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, title: title.trim(), subtitle: subtitle.trim() || undefined, category: meta.category ? category : undefined, date: date || undefined, url: meta.link ? url.trim() || undefined : undefined, imageUrl: meta.visual ? url.trim() || undefined : undefined, completedDates: target.key === "habits" ? [] : undefined, createdAt: new Date().toISOString() }, ...records]); setTitle(""); setSubtitle(""); setDate(""); setUrl(""); setAdding(false); };
