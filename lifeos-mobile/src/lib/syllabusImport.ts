...
const MODULE_ITEM = /^(\d+)\s*[-–.]\s*(\d+)\s+(.+)$/;
const MODULE_HEADER = /^(?:module\s+)?(one|two|three|four|five|six|seven|eight|nine|ten|\d+)\b/i;
...
export function parseSyllabusText(raw: string, options: ParseSyllabusOptions = {}): SyllabusItem[] {
  ...
  for (const line of lines) {
    ...
    let due = rawDue;
    if (due && !dateLooksPlausible(due, termStart)) due = undefined;
...
async function extractTextFromFile(uri: string, name?: string, mimeType?: string) {
...
export async function pickSyllabusFile(options: ParseSyllabusOptions = {}) {
  ...
  return {
    ...
    text,
    items: parseSyllabusText(text, options),
  };
}
