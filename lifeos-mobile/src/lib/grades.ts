...
export type GradeCategory = {
  ...
  weight: number;
  /** Typical points for items in this category (e.g. Discussions = 20). */
...
export const DEFAULT_GRADE_CATEGORIES: GradeCategory[] = [
  { id: "assignments", name: "Assignments", weight: 30, defaultPoints: 100 },
  { id: "discussions", name: "Discussions", weight: 10, defaultPoints: 20 },
  { id: "projects", name: "Projects", weight: 15, defaultPoints: 100 },
  { id: "quizzes", name: "Quizzes", weight: 15, defaultPoints: 10 },
  { id: "exams", name: "Exams", weight: 25, defaultPoints: 100 },
...
/** Fallback when the category has no Pts default (common syllabus sizes). */
const TYPE_DEFAULT_POINTS: Record<string, number> = {
  Discussion: 20,
  Quiz: 10,
  Lab: 50,
  Assignment: 100,
...
export function categoryIdForItem(
  ...
  // Older schemes folded discussions into assignments.
  if (item.academicType === "Discussion" && categories.some((cat) => cat.id === "assignments")) {
    return "assignments";
  }
  return categories[categories.length - 1]?.id ?? "other";
}

/** Prefer the item's own pointsPossible; otherwise category Pts, then type defaults (Discussions = 20). */
export function resolvePointsPossible(
  item: Pick<GradableItem, "pointsPossible" | "gradeCategoryId" | "academicType">,
  categories: GradeCategory[],
  overridePossible?: number,
): number | undefined {
  if (typeof overridePossible === "number" && Number.isFinite(overridePossible) && overridePossible > 0) {
    return overridePossible;
  }
  if (typeof item.pointsPossible === "number" && Number.isFinite(item.pointsPossible) && item.pointsPossible > 0) {
    return item.pointsPossible;
  }
  const cat = categories.find((row) => row.id === categoryIdForItem(item, categories));
  if (typeof cat?.defaultPoints === "number" && cat.defaultPoints > 0) return cat.defaultPoints;
  if (item.academicType && TYPE_DEFAULT_POINTS[item.academicType]) {
    return TYPE_DEFAULT_POINTS[item.academicType];
  }
  return undefined;
}
